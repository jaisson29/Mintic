...
CREATE OR REPLACE VIEW AS report_customer AS SELECT c.country, ci.city, ad.address address_store, ad.phone phone_store,
	cm.first_name firstname_customer, cm.last_name lastname_customer, 
    py.amount payment_customer, st.first_name firstname_stafff,st.last_name lastname_staff, 
    fm.title title_film, fm.description description_film, lg.name language_film,
    ct.name category, ac.first_name firstname_actor, ac.last_name lastname_actor
FROM country c
